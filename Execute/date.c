#include <stdio.h>
#include <unistd.h>
#include <string.h>
#include <errno.h>
#include <wait.h>

// Write a progream where a child is create to execute a command.
// The command will tell you the date and time in Unix.
// Use execlp().
// NOTE: You need to specify the full path of the filename that gives you the date and time information.
// Announce the successful forking of a child by displaying its pid. 
int main() {
	pid_t child = fork();

	// Error forking child
	if (child < 0) {
		printf("Error forking child. [%s]\n", strerror(errno));
		perror("Error forking child");
		return 1;
	// Child forked successfully
	} else if (child == 0) {
		printf("Child successfully forked with process id [%d]\n", getpid());
		
		// Executing date command
		execl("/bin/date", "date", NULL);

		printf("execl failed. [%s]\n", strerror(errno));
		return 1;
	// Parent process
	} else {
		wait(&child);
		printf("Back to the Parent process with process id [%d]\n", getpid());
	}
	return 0;
}